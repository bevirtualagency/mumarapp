<table id="summery" class="table table-bordered table-striped">
    <tr>
        ...
        <td style="width:60%">
           <span class="text-muted"> {{ $schedule_user->id }} ({{ $schedule_user->email }}, {{ $schedule_user->id }}</span>
        </td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td>
            <span class="text-muted"> {{ showDateTime( $schedule_user->id, $campaign->send_datetime)}} </span>
        </td>
    </tr>
    <tr>
        <td> {{trans('statistics.detail.finished_time')}} </td>
        <td>
            <span class="text-muted"> {{ showDateTime($schedule_user->id, $campaign->updated_at)}} </span>
        </td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td>
            <span class="text-muted"> On {{ showDateTime($schedule_user->id, $campaign->created_at)}} </span>
        </td>
    </tr>
</table>